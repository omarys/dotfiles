...
PLUGIN_ARMS = (
    "ponytail",
    "caveman",
)  # arms activated via --plugin-dir (vs raw --append prompts)
...
def _plugin_dir(name):
    ...
    if not versions:
        sys.exit(
            f"{name} plugin dir not found under {base}; install the plugin or set {name.upper()}_PLUGIN_DIR"
        )
...
def _selfcheck_split(p: Path):
    ...
    for i, ln in enumerate(lines):
        if ln[:1] not in (" ", "\t") and (
            ln.startswith("if __name__") or ln.startswith(_SELFCHECK_DEFS)
        ):
            start = i
            break
...
def code_stats(workdir: Path, selfcheck_as_test: bool = False):
    ...
    return {
        ...
        "src_files": len(src),
        "total_loc": sum(
            _count(p, True) for p in src
        ),  # incl comments + docstrings (the bloat)
    ...
    for tid, task in TASKS.items():
        ...
        for kind in ("good", "bad"):
            with tempfile.TemporaryDirectory() as d:
                for fn, content in task.get(
                    "seed", {}
                ).items():  # seed siblings (a helper module
                    (Path(d) / fn).write_text(
                        content, encoding="utf-8"
                    )  # the ref imports) too
                (Path(d) / task["file"]).write_text(
                    task[kind], encoding="utf-8"
                )  # entry = the ref
                r = task["score"](Path(d))
            ok = (
                (r["correct"] == 1 and r["safe"] == 1)
                if kind == "good"
                else (r[axis] == 0)
            )
            print(
                f"{'ok ' if ok else 'XX '} {tid:12} {kind:4} correct={r['correct']} "
...
def run_cell(task_id, arm, model, workdir: Path):
    ...
    if arm in PLUGIN_ARMS:
        cmd += [
            "--plugin-dir",
            _plugin_dir(arm),
        ]  # real activation of exactly one plugin
    else:
...
def aggregate(results):
    ...
    for (t, a, m), cells in sorted(groups.items()):
        ...
        loc_cells = [
            c for c in cells if c.get("total_loc", 0) > 0
        ]  # LOC only where code was delivered
        nl = len(loc_cells)
        rows.append(
            {
                "task": t,
                ...
                "total_loc_median": statistics.median(c["total_loc"] for c in loc_cells)
                if nl
                else 0,
                "src_loc_median": statistics.median(c["src_loc"] for c in loc_cells)
                if nl
                else 0,
                "total_loc_max": max((c["total_loc"] for c in loc_cells), default=0),
                "src_files_median": statistics.median(c["src_files"] for c in loc_cells)
                if nl
                else 0,
                "wrote_tests_rate": round(
                    sum(1 for c in cells if c.get("test_files", 0) > 0) / n, 3
                ),
                ...
                "total_tokens_mean": (
                    round(
                        statistics.mean(
                            [
                                (c.get("in_tokens") or 0)
                                + (c.get("out_tokens") or 0)
                                + (c.get("cache_tokens") or 0)
                                for c in cells
                                if c.get("out_tokens") is not None
                            ]
                        )
...
def print_table(rows):
    ...
    for (task, model), rs in sorted(by.items()):
        print(f"\n=== {task}  ({model}, n={rs[0]['n']}) ===")
        print(
            f"  {'arm':16} {'wrote%':>7} {'correct':>8} {'LOC':>7} {'tot_tok':>9} {'$/run':>8} {'time_s':>7}"
        )
        for r in sorted(rs, key=lambda x: x["arm"]):
            c = (
                ("$" + format(r["cost_mean"], ".4f"))
...
def _claude_version():
    try:
        return subprocess.run(
            [shutil.which("claude"), "--version"], capture_output=True, text=True
        ).stdout.strip()
    except Exception:
        return "unknown"
