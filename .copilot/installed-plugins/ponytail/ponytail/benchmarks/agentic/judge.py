...
def selftest(key):
    ...
    for task_id in ("cache", "safe-path"):
        ...
        if not (
            ...
            print(
                f"ok {task_id}: over({hi['over_engineering']}) > minimal({lo['over_engineering']})"
            )
...
def run(run_dir, key):
    ...
    for i, (tid, arm, model, ws) in enumerate(cells, 1):
        s = parse_score(judge_call(TASKS[tid]["prompt"], source_text(ws), key)) or {
    ...
    worst = sorted(
        [
            r
            for r in scored
            if isinstance(r["over_engineering"], int) and r["over_engineering"] >= 2
        ],
    ...
    for r in worst[:20]:
        print(
            f"  {r['task']:11} {r['arm']:15} {r['model']:7} score={r['over_engineering']} cite={r['cite']}"
        )
