...
def _rank_ok(scores):
    ...
    for task_id in sorted({t for (t, _) in scores}):
        ...
        if not (
            isinstance(hi.get(SCORE_KEY), int)
            and isinstance(lo.get(SCORE_KEY), int)
...
PAIRS = [
    (t, lbl, code)
    for t in STUBS
    for lbl, code in (("complete", TASKS[t]["good"]), ("stub", STUBS[t]))
]
...
def selftest(key):
    ...
    for task_id, label, code in PAIRS:
        s = parse_complete(
            judge_call(TASKS[task_id]["prompt"], code, key, system=RUBRIC)
        )
...
def run(run_dir, key):
    ...
    for i, (tid, arm, model, ws) in enumerate(cells, 1):
        s = parse_complete(
            judge_call(TASKS[tid]["prompt"], source_text(ws), key, system=RUBRIC)
        ) or {SCORE_KEY: None}
        scored.append(
            {
                "task": tid,
        ...
        (run_dir / "completeness.json").write_text(
            json.dumps(
                {"judge": JUDGE_MODEL, "rubric": RUBRIC, "scores": scored}, indent=2
            ),
            encoding="utf-8",
        )
    ...
    under = sorted(
        [
            r
            for r in scored
            if isinstance(r[SCORE_KEY], int) and r[SCORE_KEY] <= FLAG_AT
        ],
    ...
    for r in under[:20]:
        print(
            f"  {r['task']:13} {r['arm']:15} {r['model']:7} score={r[SCORE_KEY]} missing={r['missing']}"
        )
...
def main():
    ...
    ap.add_argument(
        "--selftest", action="store_true", help="live: judge ranks complete > stub"
    )
    ap.add_argument(
        "--selftest-offline", action="store_true", help="gate logic only, no API"
    )
