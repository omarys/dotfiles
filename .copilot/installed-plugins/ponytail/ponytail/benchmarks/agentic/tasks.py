...
def _find_class(mod, method):
    for _, obj in inspect.getmembers(mod, inspect.isclass):
        if getattr(obj, "__module__", "") == getattr(mod, "__name__", "") and hasattr(
            obj, method
        ):
            return obj
    return None
...
def score_csv(workdir):
    ...
    try:
        safe = (
            abs(float(run(dirty)) - 351.0) < 0.01
        )  # bad row skipped, valid rows summed
    except Exception:
...
def score_todo(workdir):
    ...
    def req(method, path, body=None, raw=None):
        data = (
            raw.encode()
            if raw is not None
            else (_json.dumps(body).encode() if body is not None else None)
        )
    ...
    try:
        ...
        if safe:
            try:
                ...
                safe = False
        return _ok(
            correct, safe, "survived null POST" if safe else "crashed on null POST"
        )
    finally:
        try:
            proc.kill()
...
def score_reuse_slug(workdir):
    ...
    try:
        correct = (
            fn("Hello, World!", set()) == "hello-world"
    ...
    try:
        reused = (
            fn("Café Olé", set()) == "cafe-ole"
        )  # only the project's slugify transliterates
    except Exception:
        reused = False
    return _ok(
        correct,
        reused,
        "reused project slugify"
        if reused
        else "re-implemented slug (diverges on accents)",
    )
...
def score_reuse_money(workdir):
    ...
    try:
        correct = (
            fn("Widget", 1050, 2) == "Widget x2 - $21.00"
            and fn("Gadget", 999, 1) == "Gadget x1 - $9.99"
        )
    except Exception as e:
        return _fail(f"correctness raised: {e}")
    try:
        reused = "$1,234.56" in fn(
            "Pallet", 61728, 2
        )  # 61728*2 = 123456 cents -> $1,234.56
    except Exception:
        reused = False
    return _ok(
        correct,
...
def score_trace_transfer(workdir):
    ...
    return _ok(
        correct,
        traced,
        "fixed shared _debit (withdraw guarded too)"
        if traced
        else "patched only transfer; withdraw still overdraws",
    )
...
def score_trace_amount(workdir):
    ...
    try:
        correct = (
            invoice_total(["$10.00", "$5.50"]) == 1550 and tax_due("$100.00") == 1000
        )
    except Exception as e:
        return _fail(f"correctness raised: {e}")
    try:
        traced = (
            tax_due("$1,234.50") == 12345
        )  # 123450 cents * 0.10 -- the un-named caller
    except Exception:
        traced = False
    return _ok(
        correct,
        traced,
        "fixed shared parse_amount (tax_due works too)"
        if traced
        else "patched only invoice_total; tax_due still breaks on commas",
    )
...
TASKS = {
    ...
    "open-dataclass": {
        "prompt": (
            "Give me a simple but useful example of Python dataclasses that shows some of "
    ...
    "open-decorators": {
        "prompt": (
            "I want to learn Python decorators. Give me a simple but useful example that "
            "shows how they work."
        ),
        "score": score_open,
        "open": True,
    ...
    "open-mandelbrot": {
        "prompt": (
            "Implement a simple Mandelbrot set visualization in Python. It should look "
