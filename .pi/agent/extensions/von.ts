...
const DEFAULT_CONFIG: VonConfig = {
  enabled: true,
  endpoint:
    process.env.VON_API_BASE ||
    process.env.TYPESAFE_API_BASE ||
    "http://127.0.0.1:8000",
  enableGating: false,
  timeoutMs: 3000,
};
...
function loadConfig(ctx: any): VonConfig {
  ...
  if (process.env.VON_ENABLED !== undefined) {
    DEFAULT_CONFIG.enabled =
      process.env.VON_ENABLED === "1" || process.env.VON_ENABLED === "true";
  }
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    description:
      "Performs rapid, calibrated non-autoregressive decision making using local Von (Jev drop-in). Modes: 'choice' (select from options), 'noul' (yes/no probability), or 'score' (0-10 numerical scoring).",
    parameters: Type.Object({
      mode: Type.Union(
        [Type.Literal("choice"), Type.Literal("noul"), Type.Literal("score")],
        { description: "Evaluation primitive mode" },
      ),
      query: Type.String({
        description: "Question or decision criteria to evaluate",
      }),
      state: Type.Optional(
        Type.String({
          description:
            "Context or state payload to judge (keep < 512 tokens for Von)",
        }),
      ),
      options: Type.Optional(
        Type.Array(Type.String(), {
          description: "Options to select from (required for 'choice' mode)",
        }),
      ),
    }),
  ...
  pi.registerCommand("von", async (args, ctx) => {
    ...
    switch (sub) {
      ...
      case "enable": {
        ...
        saveConfig(ctx, config);
        ctx.ui.notify(
          `Von integration ENABLED (endpoint: ${config.endpoint}).`,
          "info",
        );
        break;
      }
      ...
      case "toggle": {
        ...
        saveConfig(ctx, config);
        ctx.ui.notify(
          `Von is now ${config.enabled ? "ENABLED" : "DISABLED"}.`,
          "info",
        );
        break;
      }
      ...
      case "gate": {
        if (rest === "on") {
          ...
          ctx.ui.notify(
            `Usage: /von gate on | /von gate off (currently: ${config.enableGating ? "on" : "off"})`,
            "info",
          );
        }
        break;
      ...
      case "test": {
        ...
        try {
          ...
          ctx.ui.notify(
            `Von OK (${elapsed}ms): ${JSON.stringify(res)}`,
            "info",
          );
        } catch (err: any) {
      ...
      try {
        const check = await queryVon(
          config.endpoint,
          {
            mode: "noul",
            query:
              "Is this shell command potentially destructive or risky to execute without asking?",
            state: command,
          },
          1500,
        );
        ...
        if (check.probability > 0.75) {
          ctx.ui.notify(
            `Von Gate Warning: Flagged potentially risky command (${Math.round(check.probability * 100)}% risk)`,
            "warning",
          );
        }
      } catch {
